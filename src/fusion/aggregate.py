...
Phase 1 fusion vector layout:
  [text_pooled (768) | audio_pooled (768) | metadata (3)]
  Total input dim = 1539

When video is added in Phase 2, update FUSION_INPUT_DIM and insert
the video pooled vector between audio and metadata:
  [text_pooled (768) | audio_pooled (768) | video_pooled (960) | metadata (3)]
  Total input dim = 2499
"""
import numpy as np

TEXT_DIM = 768
AUDIO_DIM = 768
METADATA_DIM = 3  # keep in sync with metadata_pipeline.EMBED_DIM

# Phase 1: text + audio + metadata
FUSION_INPUT_DIM = TEXT_DIM + AUDIO_DIM + METADATA_DIM  # 1539
